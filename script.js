const months = [
  ...
  function total() {
    console.log("Total number of months : ", months.length) 
}
total();
  ...
  const earningsArray = months.map((el) => el[1]);
  
  const profitMonths = months.filter((el) => el[1] > 0);
  const salesOnProfitMonths = profitMonths
    .map((el) => el[1])
    .reduce((accVal, curVal) => accVal + curVal, 0);
  
  const avgOfProfitAndLoss =
    earningsArray.reduce((accVal, curVal) => accVal + curVal, 0) / months.length; // get the average of all total and losses
